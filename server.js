...
const ProofHash =
  mongoose.models.ProofHash || mongoose.model('ProofHash', proofHashSchema);
...
const AdminNotification =
  mongoose.models.AdminNotification || mongoose.model('AdminNotification', adminNotifSchema);
...
const MONGODB_URI =
  process.env.MONGODB_URI || 'mongodb://localhost:27017/livestockmart';
...
// -------------------------------------
// Multer (now supports multiple livestock images)
const upload = multer({
    ...
    if (expiredOrders.length > 0) {
      ...
      for (const order of expiredOrders) {
        ...
        await AdminNotification.create({
          message: `System: Order #${order._id
            .toString()
            .slice(-6)} auto-expired (unpaid > 30m).`,
          type: 'warning',
          orderId: order._id
        });
      }
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ user: { id: newUser._id, name: newUser.name, email: newUser.email } });
  } catch (err) {
...
app.get('/api/livestock', async (req, res) => {
  try {
    const livestock = await Livestock.find({}, '-images');
    res.json(livestock);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
// IMAGE BY INDEX
app.get('/api/livestock/:id/image/:idx', async (req, res) => {
  try {
    const { id, idx } = req.params;
    if (!mongoose.Types.ObjectId.isValid(id))
      return res.status(404).send('Invalid ID');

    const doc = await Livestock.findById(id, 'images');
    if (!doc || !doc.images || !doc.images.length)
      return res.status(404).send('Image not found');

    const i = Number(idx) || 0;
    const img = doc.images[i] || doc.images[0];
...
app.get('/api/livestock/image/:id', async (req, res) => {
  try {
    ...
    if (!livestock?.images?.length) return res.status(404).send('Image not found');
...
app.get('/api/admin/livestock', async (req, res) => {
  try {
    const livestock = await Livestock.find({}, '-images').sort({ createdAt: -1 });
    res.json({ livestock });
  } catch (err) {
...
app.put('/api/admin/livestock/:id', upload.array('images', 5), async (req, res) => {
  try {
    ...
    const livestock = await Livestock.findByIdAndUpdate(req.params.id, updates, {
      new: true
    });
    res.json(livestock);
  } catch (err) {
      ...
      $push: {
        notifications: {
          ...
          message: `Order #${order._id
            .toString()
            .slice(-6)} rejected: ${reason}. Items have been restocked.`,
          icon: 'x-circle',
          color: 'red',
          timestamp: Date.now(),
          seen: false
  ...
  async (req, res) => {
    try {
      ...
      if (existingProof && existingProof.orderId.toString() !== req.params.id) {
        return res
          .status(400)
          .json({ message: 'Duplicate proof detected! This image has already been used.' });
      }
      ...
      await AdminNotification.create({
        message: `Proof Re-uploaded for Order #${order._id
          .toString()
          .slice(-6)} by ${req.user.name}`,
        type: 'info',
        orderId: order._id
      });
...
// Invoice route – keep your existing implementation here

// --- PAYMENT ROUTES ---
app.post('/api/payment/create', authMiddleware, (req, res) => {
  const amount = req.body;
  const paymentId = 'PAY' + Date.now();
  const upiString = `upi://pay?pa=${process.env.UPI_ID ||
    'sai.kambala@ybl'}&pn=LivestockMart&am=${amount}`;
